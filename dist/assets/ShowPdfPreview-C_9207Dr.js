...
const j = () =>
  ...
  v = () => {
    const [l, h] = a.useState(""),
      [d, n] = a.useState(!1),
      [s] = f(),
      { setIsSessionExpired: t, setIsAuth: i } = p(),
      r = m(),
      w = u(),
      o = s.get("Srno"),
      c = s.get("financial_year"),
      L = s.get("workCode");
    return (
      a.useEffect(() => {
        async function x() {
          try {
            n(!0);
            const e = await fetch(
              `/api/v1/document/generate-pdf/?Srno=${o}&financial_year=${c}&workCode=${L}`,
              { credentials: "include" }
            );
            if ((console.log(e.status), e.status === 401)) t(!0), i(!1);
            else if (e.status === 404) {
              alert("requested document not found"), r("/");
              return;
            } else {
            ...
            console.error("An error occurred while fetching the PDF:", e);
          } finally {
            n(!1);
          }
        }
        x();
      }, [o, c, i, L, r, w.search, t]),
      d
        ? C.jsx(j, {})
        : C.jsxs("div", {
            children: [
